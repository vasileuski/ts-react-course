...
// const allGenres = (movies: Movies[]): string[] => {
//     const newArr = []
//     return movies.map(({ genre }) => {
//         return genre.map((el, item) => {
//             return newArr.push(el)
//         })
//     })
// };

// console.log(allGenres(movies))

// 3. Отсортировать фильмы по рейтингу по убыванию

const sortedMovies = (movies: Movies[]): Movies[] => {
    return movies.sort((a, b) => { return b.imdbRating - a.imdbRating })
}

// 4. Создать новый массив, где объекты фильмов будут состоять из следующих полей: id, title, released, plot

const transformArray = (movies: Movies[]): {}[] => {
...
const getMovieByYear = (movies: Movies[], number: number): {}[] => {
...
const getMovieByWord = (movies: Movies[], str: string): {}[] => {
...
const getMovieByWord2 = (movies: Movies[], str: string): {}[] => {
    return movies.filter(({ title, plot }) => {
...
const getMovie = (movies: Movies[], key: string, value: string | number): {}[] => {
